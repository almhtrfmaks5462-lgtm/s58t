...
namespace SaifcordInstaller
{
    ...
    public class LauncherForm : Form
    {
        ...
        private SaifcordBackend _backend;

        public LauncherForm()
        {
            this.Text = "Saifcord Installer";
            this.Size = new Size(740, 620);
            this.FormBorderStyle = FormBorderStyle.None;
            this.StartPosition = FormStartPosition.CenterScreen;
            this.BackColor = Color.FromArgb(20, 20, 30); // لون خلفية غامق
            
            var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("SaifcordInstaller.icon.ico");
        ...
        {
            var userDataFolder = Path.Combine(Path.GetTempPath(), "SaifcordInstaller_WebView2");
            ...
            catch (Exception ex)
            {
                ...
                {
                    MessageBox.Show(
                        "Microsoft Edge WebView2 Runtime is required to run Saifcord Installer but was not found.\n\n" +
                        "Please download and install it from:\nhttps://aka.ms/webview2\n\n" +
                        "After installing, restart Saifcord Installer.",
            ...
            _backend = new SaifcordBackend(this, _webView);
            ...
            using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("SaifcordInstaller.index.html"))
            using (var reader = new StreamReader(stream))
            {
                var html = reader.ReadToEnd();

                try {
                    using (var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("SaifcordInstaller.icon.ico"))
                    {
    ...
    public class SaifcordBackend
    {
        ...
        public SaifcordBackend(LauncherForm form, WebView2 webView)
        {
            _form = form;
            _webView = webView;
            ...
            _distDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Saifcord", "dist");
        }
        ...
        private void InstallCustomPlugins()
        {
            try
            {
                SetStatus("loading", "📦 Installing custom plugins...");
                
                string appDataPath = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
                string pluginsFolder = Path.Combine(appDataPath, "BetterDiscord", "plugins");
                
                if (!Directory.Exists(pluginsFolder))
                {
                    Directory.CreateDirectory(pluginsFolder);
                    SetStatus("info", "   Created BetterDiscord plugins directory");
                }
                
                int copiedCount = 0;
                List<string> pluginNames = new List<string>();
                
                string localPluginsDir = Path.Combine(_exeDir, "plugins");
                if (Directory.Exists(localPluginsDir))
                {
                    var pluginFiles = Directory.GetFiles(localPluginsDir, "*.plugin.js");
                    foreach (var plugin in pluginFiles)
                    {
                        string destPath = Path.Combine(pluginsFolder, Path.GetFileName(plugin));
                        File.Copy(plugin, destPath, true);
                        pluginNames.Add(Path.GetFileNameWithoutExtension(plugin));
                        copiedCount++;
                        SetStatus("info", $"   ✅ Copied: {Path.GetFileName(plugin)}");
                    }
                }
                
                string resourcesPluginsDir = Path.Combine(_exeDir, "resources", "plugins");
                if (!Directory.Exists(resourcesPluginsDir))
                {
                    resourcesPluginsDir = Path.Combine(_exeDir, "..", "resources", "plugins");
                }
                
                if (Directory.Exists(resourcesPluginsDir) && resourcesPluginsDir != localPluginsDir)
                {
                    var pluginFiles = Directory.GetFiles(resourcesPluginsDir, "*.plugin.js");
                    foreach (var plugin in pluginFiles)
                    {
                        string destPath = Path.Combine(pluginsFolder, Path.GetFileName(plugin));
                        File.Copy(plugin, destPath, true);
                ...
                try
                {
                    ...
                    {
                        if (resourceName.Contains("plugins") && resourceName.EndsWith(".plugin.js"))
                        {
                            ...
                            if (resourceName.Contains("."))
                            {
                                ...
                                {
                                    if (i + 2 < parts.Length && parts[i + 1] == "plugin" && parts[i + 2] == "js")
                                    {
                                        fileName = parts[i] + ".plugin.js";
                                        break;
                                    }
                                }
                            }
                            ...
                            using (FileStream fileStream = File.Create(destPath))
                            {
                                if (stream != null)
                                {
                                    ...
                                    if (!pluginNames.Contains(pluginName))
                                    {
                                        pluginNames.Add(pluginName);
                                        copiedCount++;
                                        SetStatus("info", $"   ✅ Copied (embedded): {fileName}");
                                    }
                                }
                            }
                        }
                ...
                if (copiedCount > 0)
                {
                    SetStatus("info", $"✅ Successfully installed {copiedCount} plugin(s)");
                    EnablePluginsAutomatically(pluginsFolder, pluginNames);
                }
                else
                {
                    SetStatus("info", "ℹ️ No custom plugins found to install");
                }
            }
            catch (Exception ex)
            {
                SetStatus("error", $"⚠️ Failed to install plugins: {ex.Message}");
            }
        }
        
        private void EnablePluginsAutomatically(string pluginsFolder, List<string> pluginNames)
        {
            try
            {
                string betterDiscordFolder = Path.GetDirectoryName(pluginsFolder);
                string configPath = Path.Combine(betterDiscordFolder, "plugins.json");
                ...
                {
                    if (!config.ContainsKey(pluginName) || !(config[pluginName] is true))
                    {
                        config[pluginName] = true;
                        SetStatus("info", $"   🔧 Enabled: {pluginName}");
                    }
                }
                
                string updatedJson = JsonSerializer.Serialize(config, new JsonSerializerOptions { WriteIndented = true });
                File.WriteAllText(configPath, updatedJson);
                SetStatus("info", "✅ Plugins configuration saved");
            }
            catch (Exception ex)
            {
                SetStatus("info", $"⚠️ Could not auto-enable plugins: {ex.Message}");
            }
        }
        // ======================================
        ...
        public async Task<bool> IsInjected(string path)
        {
            return await Task.Run(() => {
                ...
                return Directory.Exists(appDir) && File.Exists(pkgPath) && File.ReadAllText(pkgPath).Contains("\"saifcord\"");
            });
        }
        ...
        public async Task<bool> HasThirdPartyMod(string path)
        {
            return await Task.Run(() => {
                ...
                if (content.Contains("\"saifcord\"")) return false;
        ...
        private string _injectJobState = "idle";
        ...
        private async Task EnsureDistAsync()
        {
            ...
            _http.DefaultRequestHeaders.Add("User-Agent", "Saifcord-Installer/1.0");
            ...
            {
                throw new Exception("API timed out (30s). Check your internet connection and try again.");
            }
            ...
            if (string.IsNullOrEmpty(zipUrl))
                throw new Exception($"'{DIST_ZIP}' not found in the release.");

            SetProgress(5, "Starting download...");
            var tmpZip = Path.Combine(Path.GetTempPath(), "saifcord-dist.zip");
                ...
                using (var fs = new FileStream(tmpZip, FileMode.Create, FileAccess.Write, FileShare.None, 81920, true))
                {
                    ...
                    while ((read = await contentStream.ReadAsync(buffer, 0, buffer.Length)) > 0)
                    {
                        ...
                        if (percent - lastReportedPercent >= 0.5 || percent >= 100.0)
                        {
                            ...
                            SetProgress(overallPercent, "Downloading Saifcord...", readMB, totalMB);
                        }
                    }
                }
            }
        ...
        private void DoInject(string resPath)
        {
            ...
            SetProgress(91, "Removing previous mod injection...");
            ...
            SetProgress(92, "Configuring Saifcord loader...");
        ...
        private void CleanModulePatches(string resPath)
        {
            ...
            {
                Console.WriteLine($"[Saifcord] CleanModulePatches warning: {ex.Message}");
            }
        }
        ...
        private void DoUninject(string resPath)
        {
            ...
            if (Directory.Exists(appDir))
            {
                ...
                if (File.Exists(pkg) && File.ReadAllText(pkg).Contains("\"saifcord\""))
                {
        ...
        private void WriteLoader(string appDir)
        {
            ...
            File.WriteAllText(Path.Combine(appDir, "package.json"), "{\"name\":\"saifcord\",\"main\":\"index.js\"}");
            File.WriteAllText(Path.Combine(appDir, "index.js"),
                ...
                $"if (!fs.existsSync(patcherPath)) throw new Error('[Saifcord] patcher.js not found. Expected at: ' + primary);\n" +
                $"require(patcherPath);\n"
            );
        }
