<script>
    ...
    function readLicenseFile() {
        fs.readFile(path.join(__static, "/license.txt"), (err, data) => {
            if (err) return licenseText = "راجع الترخيص على https://git.nightcord.su/nightcord/nightcord/src/branch/master/LICENSE";
            licenseText = data;
        });
    }
...
<section class="page" in:page="{{duration: $hasLoaded ? undefined : 0}}" out:page="{{out: true}}">
    ...
    <Checkbox checked={$hasAgreed} disabled={!licenseText} label="أوافق على اتفاقية الترخيص." on:change={toggleAgree} />
</section>