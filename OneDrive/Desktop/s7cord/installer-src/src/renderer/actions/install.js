...
// ✅ تغيير: رابط API إلى GitHub حق S7Cord
            ...
            for (const d of dirs) {
                ...
                for (const pf of patchedFiles) {
                    ...
                    const isPatched = content.toLowerCase().includes("s7cord") ||
                                      content.toLowerCase().includes("vencord") ||
                ...
                if (await safeExists(innerAppDir)) {
                    ...
                    if (await safeExists(innerPkg)) {
                        ...
                        const isMod = pkgContent.toLowerCase().includes("s7cord") ||
                                      pkgContent.toLowerCase().includes("vencord") ||
...
async function injectShims(paths) {
    ...
    for (const resPath of paths) {
        ...
        try {
            ...
            log("1. Removing previous mod injection (S7Cord / Vencord / Equicord / OpenAsar)...");
