# build-installer.ps1 — Build S7Cord-Installer.exe (Electron + electron-builder)
...
$OutExe    = Join-Path $OutDir "S7Cord-Installer.exe"

Write-Host ""
Write-Host "  [S7Cord] Building Electron installer..." -ForegroundColor Cyan
...
if (Test-Path $OutExe) {
    ...
    Write-Host ""
    Write-Host "  OK  S7Cord-Installer.exe compile ($size KB)" -ForegroundColor Green
    Write-Host "    -> $OutExe" -ForegroundColor DarkGray
    Write-Host ""
} else {
    Write-Host "  [ERREUR] S7Cord-Installer.exe introuvable apres compilation." -ForegroundColor Red
    exit 1
}
