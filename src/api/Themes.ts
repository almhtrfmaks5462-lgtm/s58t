...
import { ThemeStore } from "@S7Cord/discord-types";
import { PopoutWindowStore } from "@webpack/common";

import { userStyleRootNode, S7CordRootNode } from "./Styles";
...
async function toggle(isEnabled: boolean) {
    if (!style) {
        if (isEnabled) {
            style = createAndAppendStyle("S7Cord-custom-css", userStyleRootNode);
            S7CordNative.quickCss.addChangeListener(css => {
                style.textContent = css;
                // At the time of writing this, changing textContent resets the disabled state
                style.disabled = !Settings.useQuickCss;
                updatePopoutWindows();
            });
            style.textContent = await S7CordNative.quickCss.get();
        }
    } else
...
async function initThemes() {
    themesStyle ??= createAndAppendStyle("S7Cord-themes", userStyleRootNode);
    ...
    if (IS_WEB) {
        for (const theme of enabledThemes) {
            const themeData = await S7CordNative.themes.getThemeData(theme);
            if (!themeData) continue;
            const blob = new Blob([themeData], { type: "text/css" });
            links.push(URL.createObjectURL(blob));
        }
    } else {
        const localThemes = enabledThemes.map(theme => `S7Cord:///themes/${theme}?v=${Date.now()}`);
...
function applyToPopout(popoutWindow: Window | undefined, key: string) {
    ...
    doc.querySelector("S7Cord-root")?.remove();

    doc.documentElement.appendChild(S7CordRootNode.cloneNode(true));
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!IS_WEB) {
        S7CordNative.quickCss.addThemeChangeListener(initThemes);
    }
}, { once: true });
