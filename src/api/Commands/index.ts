...
import { CommandArgument, CommandContext, CommandOption } from "@S7Cord/discord-types";

import { sendBotMessage } from "./commandHelpers";
import { ApplicationCommandInputType, ApplicationCommandOptionType, ApplicationCommandType, S7CordCommand } from "./types";

export * from "./commandHelpers";
export * from "./types";

export let BUILT_IN: S7CordCommand[];
export const commands = {} as Record<string, S7CordCommand>;
...
export const _init = function (cmds: S7CordCommand[]) {
    try {
        BUILT_IN = cmds;
...
export const _handleCommand = function (cmd: S7CordCommand, args: CommandArgument[], ctx: CommandContext) {
    if (!cmd.isS7CordCommand)
 ...
 * @param opt
 */
export function prepareOption<O extends CommandOption | S7CordCommand>(opt: O): O {
...
// investigate
function registerSubCommands(cmd: S7CordCommand, plugin: string) {
...
export function registerCommand<C extends S7CordCommand>(command: C, plugin: string) {
    if (!BUILT_IN) {
        console.warn(
            "[CommandsAPI]",
