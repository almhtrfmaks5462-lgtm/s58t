...
function makeShortcuts() {
    ...
    return {
        ...
        wpPatcher: { getter: () => S7Cord.WebpackPatcher },
        wpInstances: { getter: () => S7Cord.WebpackPatcher.allWebpackInstances },
        wpsearch: search,
        wpex: extract,
        ...
        PluginsApi: { getter: () => S7Cord.Plugins },
        plugins: { getter: () => S7Cord.Plugins.plugins },
        Settings: { getter: () => S7Cord.Settings },
        Api: { getter: () => S7Cord.Api },
        Util: { getter: () => S7Cord.Util },
            ...
            if (!win.prepared) {
                ...
                [...document.querySelectorAll("style"), ...document.querySelectorAll("link[rel=stylesheet]")].forEach(s => {
                    ...
                    if (s.parentElement?.tagName === "HEAD")
                        doc.head.append(n);
                    else if (n.id?.startsWith("S7Cord-") || n.id?.startsWith("vcd-"))
        ...
        preEnable: (plugin: string) => (S7Cord.Settings.plugins[plugin] ??= { enabled: true }).enabled = true,
...
function loadAndCacheShortcut(key: string, val: any, forceLoad: boolean) {
    ...
    function unwrapProxy(value: any) {
        ...
        } else if (value.$$S7CordGetWrappedComponent) {
            return forceLoad ? value.$$S7CordGetWrappedComponent() : value;
        }
...
export default definePlugin({
    ...
    start() {
        ...
        if (!IS_WEB) {
            const Native = S7CordNative.pluginHelpers.ConsoleShortcuts as PluginNative<typeof import("./native")>;
