...
import { ChromiumIcon, ClientIcon, DevBannerIcon, DiscordIcon, ElectronIcon, S7CordIcon, names, settings } from ".";
...
export function makeDevBanner(state?: string): string | JSX.Element {
    ...
    const format = settings.store.format ?? "{devbannerIcon} {buildChannel} {buildNumber} ({buildHash}) | {S7CordIcon} {S7CordName} {S7CordVersion} ({S7CordHash})";
    ...
    const replaced = baseFormat
        ...
        .replace(/{S7CordVersion}/g, VERSION)
        .replace(/{S7CordHash}/g, gitHashShort)
        .replace(/{S7CordPlatform}/g, getVersionInfo(false))
    ...
    const parts = replaced.split(/({.*?}|__NEWLINE__)/).filter(Boolean).map((part, i) => {
        switch (part) {
            ...
            case "{S7CordIcon}":
                return <span key={`icon-S7Cord-${i}`} className="vc-discord-dev-banner-icons"><S7CordIcon /></span>;
            case "{electronIcon}":
