/*
 * S7Cord — Local un-injector for Discord Desktop
...
function findAllDiscordResources() {
    ...
    // Filtrer uniquement les paths avec une injection S7Cord présente
...
function uninject(resourcesDir) {
    ...
    // Vérifier que le dossier app/ a bien été créé par S7Cord
    if (existsSync(appDirPath)) {
        try {
            const pkg = JSON.parse(readFileSync(join(appDirPath, "package.json"), "utf-8"));
            if (pkg.name !== "S7Cord") {
                console.warn(`\x1b[33m[S7Cord] Le dossier app/ existe mais n'a pas été créé par S7Cord (name: "${pkg.name}").\x1b[0m`);
                console.warn("\x1b[33m            Abandon pour éviter de casser un autre mod.\x1b[0m");
                return false;
            }
        } catch { }

        console.log("[S7Cord] Suppression du dossier app/ injecté...");
        rmSync(appDirPath, { recursive: true, force: true });
    } else {
        console.log("\x1b[33m[S7Cord] Aucun dossier app/ injecté trouvé.\x1b[0m");
    }

    // Restaurer le backup
    if (existsSync(backupPath) && !existsSync(appAsarPath)) {
        console.log("[S7Cord] Restauration _app.asar → app.asar...");
        renameSync(backupPath, appAsarPath);
    } else if (existsSync(backupPath) && existsSync(appAsarPath)) {
        console.log("[S7Cord] app.asar déjà présent, nettoyage du backup...");
        rmSync(backupPath, { force: true });
    }

    console.log(`\x1b[32m[S7Cord] Désinjection réussie depuis : ${resourcesDir}\x1b[0m`);
...
if (allResources.length === 0) {
    console.error("\x1b[31m[S7Cord] Aucune installation Discord avec S7Cord injecté trouvée.\x1b[0m");
    console.error("\x1b[33m           Assurez-vous que S7Cord a bien été injecté via 'pnpm inject'.\x1b[0m");
    process.exit(1);
}
...
if (uninjectCount === 0) {
    console.error("\x1b[31m[S7Cord] Aucune désinjection réussie.\x1b[0m");
    process.exit(1);
}

console.log(`\n\x1b[32m[S7Cord] ${uninjectCount}/${allResources.length} désinjection(s) réussie(s).\x1b[0m`);
