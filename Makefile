...
all: $(BUILD_DIR)/S7Cord $(BUILD_DIR)/updater.exe
...
$(BUILD_DIR)/S7Cord_rc.o: S7Cord.rc assets/S7Cord.ico
	mkdir -p $(BUILD_DIR)
	windres S7Cord.rc -o $@
...
$(BUILD_DIR)/S7Cord: $(OBJS) $(BUILD_DIR)/resources.o $(RC_OBJ) $(BUILD_DIR)/themes/ $(BUILD_DIR)/sounds/
...
uninstall:
	rm -f $(PREFIX)/share/applications/S7Cord.desktop
	rm -f $(PREFIX)/share/pixmaps/S7Cord.png
	rm -f $(PREFIX)/share/pixmaps/S7Cord.svg
	rm -f $(PREFIX)/bin/S7Cord

install: uninstall
	cp S7Cord.desktop $(PREFIX)/share/applications/S7Cord.desktop
	cp assets/icon.svg $(PREFIX)/share/pixmaps/S7Cord.svg
	cp $(BUILD_DIR)/S7Cord $(PREFIX)/bin/S7Cord

run: all
	cp assets/S7Cord.gschema.xml $(BUILD_DIR)/assets/S7Cord.gschema.xml
